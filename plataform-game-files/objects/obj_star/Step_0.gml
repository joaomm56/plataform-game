...
#region estado		
	...
	#region free		
		case "free":	
			...
			if (_chao){			
				...
				if (velh != 0){
					muda_sprite(spr_star_run)
					image_xscale = sign(velh)* -0.9;
				}else muda_sprite(spr_star_idle);				
				...
				velv += grav;
				muda_sprite(spr_star_fall);				
			}
		break; 
	#endregion free	
	#region dano
		case "dano":
			muda_sprite(spr_star_hurt)
			if (instance_exists(obj_hitbox_s1))instance_destroy(obj_hitbox_s1);
	...
	#region chase
		case "chasing":
			muda_sprite(spr_star_run)
			tempo_attack = segundos * 3;
			if(dano)estado = "dano";		
			
			image_xscale = _dir;
			if (_distancia < 40)estado = "attacking";
	...
	#region attack
		case "attacking":
			if (dano)estado = "dano";			
			image_xscale = _dir;		
	                muda_sprite(spr_star_attack);
					tempo_attack -= 1;
					if (pode_cair == false)if (not place_meeting(x + velh * 15, y + 1 , obj_colisoes))velh *= -1;
					if (image_index > 1)if(!instance_exists(obj_hitbox_s1))instance_create_layer(x , y, layer,obj_hitbox_s1);
					
	                if (image_index >= image_number - 1 && tempo_attack <= 0) {	
						if (instance_exists(obj_hitbox_s1))instance_destroy(obj_hitbox_s1);	
